...
const LONG_DESCRIPTIONS: Record<string, string> = {
  ...
  check_compliance: `Pre-flight compliance triage on an IBAN before a SEPA / cross-border payment: ${BANK_LEVEL_SANCTIONS}; FATF status of the country; SEPA Instant reachability; whether the bank answers Verification of Payee (VoP) requests. Returns risk_score 0-100. Informational, not a regulated AML/CFT product. Cost: $0.02.`,
  lookup_ch_clearing: `Resolve a Swiss BC-Nummer / IID (1-5 digits) into institution name, type, address, BIC and the full payment-rail participation (SIC, RTGS CHF, Instant Payments CHF, euroSIC, LSV+/BDD) plus QR-IID, for every IID of the SIX BankMaster: ${F.claim.chClearing} entries, refreshed monthly. Cost: $0.003. Only relevant for CH/LI accounts.`,
