    ...
    schemas: {
      ...
      ComplianceResult: {
        ...
        properties: {
          ...
          flags: { type: 'array', items: { type: 'string' }, description: 'List of specific risk flags detected. Three flags carry no weight and name a check that did not happen: no_bank_resolved, sepa_register_unavailable and vop_register_unavailable. sanctions_lists_unavailable (a bank was resolved but no sanctions list is loaded on this deployment) holds the score at 50 at least. sanctions_list_unavailable_<list>, for example sanctions_list_unavailable_un, carries no weight: the bank was screened, but that one named list is not loaded on this deployment, so bank_sanctioned false says nothing about it.', example: ['fatf_grey_list', 'emi_issuer', 'no_vop'] },
        },
      },
      ChClearingResult: {
        type: 'object',
