...
describe('parseWebEvent', () => {
  ...
  it('refuses a bad name, page or locale', () => {
    ...
    // Only the page's vocabulary: a deployment probe or a made-up family is refused.
    expect(parseWebEvent({ name: 'probe:deploy', page: '/fr', locale: 'fr' })).toBeNull();
    expect(parseWebEvent({ name: 'cta:', page: '/fr', locale: 'fr' })).toBeNull();
    expect(parseWebEvent({ name: 'film:end', page: '/', locale: 'en' })?.name).toBe('film:end');
...
describe('POST /v1/web/events', () => {
  ...
  it('records a valid event and answers 204', async () => {
    ...
    const row = getStatsDB()
      .prepare('SELECT page, locale, referrer, viewport FROM web_events WHERE name = ?')
      .get(`cta:${RUN}`) as Record<string, unknown>;
  ...
  it('answers 400 to junk and 413 to a big body', async () => {
    ...
    expect(
      (await post(app, { name: 'cta:big', page: '/fr', locale: 'fr', referrer: 'a'.repeat(2000) }))
        .status,
    ).toBe(413);
  });
  ...
  it('limits one address to the window', async () => {
    ...
    for (let i = 0; i < WEB_EVENTS_PER_WINDOW; i++) {
      expect(
        (await post(app, { name: 'cta:limit', page: '/fr', locale: 'fr' }, '198.51.100.9')).status,
      ).toBe(204);
    }
    expect(
      (await post(app, { name: 'cta:limit', page: '/fr', locale: 'fr' }, '198.51.100.9')).status,
    ...
    expect(
      (await post(app, { name: 'cta:limit', page: '/fr', locale: 'fr' }, '198.51.100.10')).status,
    ).toBe(204);
  });
});
...
describe('GET /v1/admin/web-events', () => {
  ...
  it('counts by name, page, referrer and day', async () => {
    ...
    const res = await app.request('/v1/admin/web-events?days=1', {
      headers: { 'X-Admin-Secret': SECRET },
    });
    ...
    const body = (await res.json()) as {
      ...
      by_referrer: Array<{ referrer: string }>;
      by_day: Array<{ day: string }>;
    };
