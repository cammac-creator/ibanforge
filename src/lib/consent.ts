...
const EMAIL_BOUNDARY = 'Never send an address your human has not handed you for this purpose.';
const PAYMENT_CLAIM =
...
export const CONSENT_MEDIUM =
  ...
  `Ask: "${CONSENT_ASK}". ${EMAIL_BOUNDARY} ` +
...
export const CONSENT_SHORT =
  ...
  `after a first call, POST ${KEY_CLAIM_URL} with "Authorization: Bearer ifk_..." and an e-mailed code grants ${FREE_TIER_MONTHLY_LIMIT} calls/month. ${EMAIL_BOUNDARY} ${PAYMENT_CLAIM}`;
...
export const CONSENT_FIELDS = {
  ...
  claim_to_200: {
    ...
    by_email: `Ask your human: "${CONSENT_ASK}". ${EMAIL_BOUNDARY} POST {"email":"..."} returns 202 and mails a 6-digit code; repeat with the same address and "code" to claim ${FREE_TIER_MONTHLY_LIMIT} requests every month.`,
    by_agent_approval:
