...
describe('the queue of mail nobody can be attached to', () => {
  it('keeps one row per message, so a nightly re-run corrects instead of duplicating', () => {
    ...
    recordOrphan({ id: `${P}1`, sender: 'A@Example.net', msg_date: '2026-08-14 19:09', kind: 'reply' });
    recordOrphan({ id: `${P}1`, sender: 'a@example.net', subject: 'Re: IBANforge', msg_date: '2026-08-14 19:09', kind: 'reply' });
  ...
  it('lowercases the sender, since the same person writes both ways', () => {
    clean();
    recordOrphan({ id: `${P}2`, sender: '  Uwe@Example.NET ', msg_date: '2026-08-14 19:09', kind: 'reply' });
    expect(mine(getOrphans())[0].sender).toBe('uwe@example.net');
    clean();
  });

  it('puts replies above first contacts, because somebody is waiting on a reply', () => {
    clean();
    recordOrphan({ id: `${P}new`, sender: 'cold@example.net', msg_date: '2026-08-14 20:00', kind: 'first_contact' });
    recordOrphan({ id: `${P}old`, sender: 'known@example.net', msg_date: '2026-08-10 09:00', kind: 'reply' });
  ...
  it('empties when dealt with, and remembers who it belonged to', () => {
    clean();
    recordOrphan({ id: `${P}3`, sender: 'uwe@example.net', msg_date: '2026-08-14 19:09', kind: 'reply' });
  ...
  it('does not re-open a decision when the sync sees the message again', () => {
    clean();
    recordOrphan({ id: `${P}4`, sender: 'uwe@example.net', msg_date: '2026-08-14 19:09', kind: 'reply' });
    ...
    recordOrphan({ id: `${P}4`, sender: 'uwe@example.net', subject: 'Re: IBANforge', msg_date: '2026-08-14 19:09', kind: 'reply' });
    expect(mine(getOrphans())).toHaveLength(0);
