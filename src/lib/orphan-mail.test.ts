...
import { countPendingOrphans, getOrphans, isOrphanKind, recordOrphan, resolveOrphan } from './orphan-mail.js';
...
describe('the queue of mail nobody can be attached to', () => {
  it('keeps one row per message, so a nightly re-run corrects instead of duplicating', () => {
    ...
    recordOrphan({ id: `${P}1`, sender: 'A@Example.net', msg_date: '2026-01-05 10:00', kind: 'reply' });
    recordOrphan({ id: `${P}1`, sender: 'a@example.net', subject: 'Re: IBANforge', msg_date: '2026-01-05 10:00', kind: 'reply' });
  ...
  it('lowercases the sender, since the same person writes both ways', () => {
    clean();
    recordOrphan({ id: `${P}2`, sender: '  Mixed@Example.NET ', msg_date: '2026-01-05 10:00', kind: 'reply' });
  ...
  it('puts replies above first contacts, because somebody is waiting on a reply', () => {
    clean();
    recordOrphan({ id: `${P}new`, sender: 'cold@example.net', msg_date: '2026-01-06 08:00', kind: 'first_contact' });
    recordOrphan({ id: `${P}old`, sender: 'known@example.net', msg_date: '2026-01-02 09:00', kind: 'reply' });
  ...
  it('empties when dealt with, and remembers who it belonged to', () => {
    clean();
    recordOrphan({ id: `${P}3`, sender: 'sender@example.net', msg_date: '2026-01-05 10:00', kind: 'reply' });
  ...
  it('does not re-open a decision when the sync sees the message again', () => {
    clean();
    recordOrphan({ id: `${P}4`, sender: 'sender@example.net', msg_date: '2026-01-05 10:00', kind: 'reply' });
    ...
    recordOrphan({ id: `${P}4`, sender: 'sender@example.net', subject: 'Re: IBANforge', msg_date: '2026-01-05 10:00', kind: 'reply' });
    expect(mine(getOrphans())).toHaveLength(0);
  ...
  it('serves the oldest waiting rows first, so the limit can only cut the newest', () => {
    ...
    recordOrphan({ id: `${P}r-new`, sender: 'sender@example.net', msg_date: '2026-01-06 12:00', kind: 'reply' });
    recordOrphan({ id: `${P}r-old`, sender: 'sender@example.net', msg_date: '2026-01-03 12:00', kind: 'reply' });
    recordOrphan({ id: `${P}f-new`, sender: 'cold@example.net', msg_date: '2026-01-06 13:00', kind: 'first_contact' });
    recordOrphan({ id: `${P}f-old`, sender: 'cold@example.net', msg_date: '2026-01-01 13:00', kind: 'first_contact' });
    expect(mine(getOrphans()).map((r) => r.id)).toEqual([`${P}r-old`, `${P}r-new`, `${P}f-old`, `${P}f-new`]);
    clean();
  });
