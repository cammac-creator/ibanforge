...
Premier utilisateur organique réel d'IBANforge : **IB4** (`ib4.net`, secteur bancaire), via
`nc.sls@ib4.net`. Cas d'usage déclaré : *« auto filling for Odoo CRM system »* — ils
...
- Build immédiat (ne pas attendre la réponse de Nikolay), en parallèle de la relance email.
...
- **Demande réelle** : Nikolay (IB4) pourrait répondre que l'API directe lui suffit → le module
  reste un pari distribution autonome (App Store), pas une dépendance à sa réponse.
- **Version Odoo d'IB4** inconnue (18 ? 17 ?) — à confirmer via la relance ; MVP cible 18.0.
