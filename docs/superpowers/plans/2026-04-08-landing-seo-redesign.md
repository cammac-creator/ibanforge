# Landing Page Redesign + SEO — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Refresh the Hono landing page at `api.ibanforge.com` with complete SEO metadata, dev-first funnel layout, modern feature grid, dual pricing paths, and an OG image route.

**Architecture:** Single-file rewrite of `src/routes/landing.ts` (inline HTML/CSS/JS template). New `src/routes/og-image.ts` for the OG image. Minor wiring in `src/index.ts`. All changes are backend-only (Hono).

**Tech Stack:** Hono, TypeScript, inline HTML/CSS/JS, SVG icons, schema.org JSON-LD

**Spec:** `docs/superpowers/specs/2026-04-08-landing-seo-redesign.md`

---

### Task 1: Write landing page tests

**Files:**
- Create: `src/routes/landing.test.ts`

- [ ] **Step 1: Create the test file**

```typescript
// src/routes/landing.test.ts
...
Run: `npm run test -- src/routes/landing.test.ts`

Expected: Most tests FAIL (current landing has no SEO meta tags, no compliance tab, no pricing section).

- [ ] **Step 3: Commit**

```bash
git add src/routes/landing.test.ts
git commit -m "test: add landing page SEO and content tests"
```

---

### Task 2: Complete rewrite of landing.ts

This is the main task — a full rewrite of `src/routes/landing.ts` with all 6 sections: Head/SEO, Hero, Try it, Features, Pricing, Quick Start.

**Files:**
- Modify: `src/routes/landing.ts` (complete replacement)

- [ ] **Step 1: Write the complete landing.ts file**

Replace the entire file with the following. The file contains:
1. SEO `<head>` with meta tags, OG, Twitter Card, two JSON-LD schemas
2. CSS styles (all inline, modern design)
3. Hero section with badges and 3 CTAs
4. Try it demo with 3 tabs (IBAN, BIC, Compliance)
5. Features grid 3x2 with SVG icons
6. Pricing with two cards + price table
7. Quick start with 4 language tabs + MCP callout
8. JavaScript for tab switching, colorized JSON results, response time display
...
git commit -m "feat(landing): redesign with SEO, dev-first funnel, modern features grid, dual pricing"
```

---

### Task 3: Add OG image route

**Files:**
- Create: `src/routes/og-image.ts`
- Modify: `src/index.ts:16` (add import), `src/index.ts:120` (add route)
...
ogImage.get('/og-image.png', (c) => {
  ...
  <text x="600" y="400" text-anchor="middle" font-family="-apple-system,BlinkMacSystemFont,sans-serif" font-size="20" fill="#71717a">121K BICs · 75+ Countries · MCP Native · x402 Micropayments</text>
...
Add import after line 16 (after the discovery import):

```typescript
import { ogImage } from './routes/og-image.js';
```

Add route registration in the free routes section (after `app.route('/', discovery);`):
...
```bash
git add src/routes/og-image.ts src/index.ts
git commit -m "feat(seo): add OG image route for social sharing previews"
```

---

### Task 4: Run tests, verify, deploy
...
Expected: All tests PASS. If any fail, fix the issue in landing.ts and re-run.

- [ ] **Step 2: Smoke test locally**

Run: `curl -s http://localhost:3000/ | head -30`

Verify: starts with `<!DOCTYPE html>`, contains `og:title`, `121K`.

Run: `curl -s -I http://localhost:3000/`

Verify: `cache-control: public, max-age=3600`.

Run: `curl -s http://localhost:3000/og-image.png | head -3`
...
Run: `curl -s https://api.ibanforge.com/ | grep -c 'og:title'`

Expected: `1`.

Run: `curl -s -I https://api.ibanforge.com/og-image.png`
