...
```markdown
- [IBANforge](https://github.com/cammac-creator/ibanforge) - REST API and MCP server for IBAN validation (84 countries, mod-97 checksum) and BIC/SWIFT lookup (121K+ BIC entries, 38K LEI-enriched via GLEIF). x402 micropayments, self-hostable via Docker.
```
...
* [IBANforge](https://ibanforge.com) - IBAN validation API and MCP server for 84 countries with BIC/SWIFT lookup (121K+ BIC entries, 38K LEI-enriched via GLEIF). Pay-per-call x402 micropayments or self-host.
```
