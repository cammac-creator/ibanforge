...
- **4 licences fintech FINMA** actives (Customer J, Customer H, Customer I, Customer E/Customer E) -- petites structures agiles, pas de legacy IBAN validation en interne.
...
|---|---|---|---|---|---|
| **Customer H** | Zurich | Comptes business digitaux pour PME, multi-devises, paiements internationaux | Validation IBAN sur chaque virement sortant + onboarding beneficiaire. La detection vIBAN aide leur compliance AML. | ~20-40 pers., licence fintech FINMA | customer-h.example -- formulaire contact + LinkedIn founders |
| **Customer M Treasury** | Zurich | Plateforme tresorerie PME : FX, paiements internationaux, comptes multi-devises | API-first, deja integree avec des ERP. Validation IBAN + BIC lookup sur chaque paiement international. | ~30-50 pers., fondee 2014 | customer-m.example/api -- equipe tech accessible |
| **Customer J** (ex-Klarpay) | Zurich | Comptes Swiss pour business digitaux, e-commerce, createurs. IBAN personnalises, Visa. | Emetteur d'IBANs CH -- besoin de valider les IBANs recus pour les virements. Detection vIBAN cruciale pour leur clientele e-commerce. | ~30-50 pers., licence fintech FINMA, Visa principal member | customer-j.example -- PR team active, LinkedIn CEO |
| **Customer I** | Zurich | BaaS API-first, comptes avec IBANs virtuels, cartes, paiements CH et internationaux | Leur coeur de metier est l'API. IBANforge se branche directement dans leur stack. Detection vIBAN pertinente car ils emettent eux-memes des vIBANs. | ~20-40 pers., 1ere licence fintech FINMA (2020) | customer-i.example -- API docs publiques, equipe tech active sur LinkedIn |
| **Customer E / Customer E** | Zurich | Banking Web3, comptes Swiss, participant SWIFT et SEPA | Passerelle fiat/crypto -- validation IBAN necessaire pour les off-ramps. Niche mais tech-savvy. | ~15-30 pers., licence fintech FINMA | customer-e.example -- CEO Haoning Zhang (ETH, ex-Avaloq/UBS) |
...
|---|---|---|---|
| **Customer K** | AML/KYC pour institutions financieres, scoring de risque | Risk scoring pays + detection vIBAN = enrichissement direct de leur pipeline compliance | customer-k.example |
| **Customer L** | Plateforme KYC/CDD pour banques, echange securise de donnees KYC | Validation IBAN comme couche supplementaire dans le processus KYC | Via LinkedIn / site web |
| **Customer F** | Detection fraude et AML pour banques | La classification emetteur (neobank vs banque traditionnelle) aide a scorer les transactions | customer-f.example |
| **Customer G** | Compliance-as-code, regles reglementaires digitalisees (Zurich, $8.1M leves) | Integration des regles IBAN par pays dans leur moteur de compliance | customer-g.example |
...
3. [ ] **Envoyer les 5 emails Tier 1** cette semaine (Customer H, Customer M, Customer J, Customer I, Customer E)
