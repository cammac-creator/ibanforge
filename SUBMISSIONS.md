...
- lookup_bic — resolve a BIC/SWIFT code into bank name, country, city, LEI, address. Backed by 121,000+ BIC entries (39,000+ LEI-enriched via GLEIF, refreshed monthly). ($0.003)
- lookup_ch_clearing — resolve a Swiss BC-Nummer / IID into institution name, type, address, full payment-rail participation (SIC, euroSIC, CHF instant, LSV+/BDD) and QR-IID — the deepest Swiss clearing data in any public API, backed by 1,100+ SIX BankMaster entries. ($0.003)
- check_compliance — pre-flight risk triage before a SEPA payment: sanctions screening (OFAC, EU), FATF jurisdictions, SEPA Instant reachability, VoP participant. Returns risk_score 0-100. ($0.02)
...
{
  "name": "IBANforge",
  "description": "IBAN validation, BIC/SWIFT lookup, Swiss BC-Nummer, SEPA + VoP and compliance risk scoring for AI agents. 121,000+ BIC entries (39,000+ LEI-enriched via GLEIF), 1,100+ SIX BankMaster Swiss entries, x402 micropayments.",
...
- Backed by 121,000+ BIC entries (39,000+ LEI-enriched via GLEIF; other rows from the SWIFT directory, Bundesbank, SIX, EBA Step2 SCT, NBP — refreshed monthly) and 1,100+ SIX BankMaster Swiss BC-Nummern with full payment-rail participation + QR-IID (the deepest Swiss clearing data in any public API)
...
```markdown
| [IBANforge](https://ibanforge.com) | Compliance API for AI agents — IBAN validation, BIC lookup, Swiss BC-Nummer (1,100+ SIX entries), SEPA + VoP, risk scoring | $0.003–$0.02 / call | [/.well-known/x402](https://api.ibanforge.com/.well-known/x402) | Base | [GitHub](https://github.com/cammac-creator/ibanforge) |
```
...
```
IBANforge is a compliance API for AI agents — IBAN validation (mod-97 + BBAN parsing), BIC/SWIFT resolution against 121,000+ BIC entries (39,000+ LEI-enriched via GLEIF; additional rows from SWIFT directory, Bundesbank, SIX, NBP, EBA Step2 SCT), Swiss BC-Nummer / QR-IID lookup against 1,100+ SIX BankMaster entries (full payment-rail participation — the deepest Swiss clearing data in any public API), EMI/vIBAN issuer classification, SEPA Instant + VoP (EU 2024/886) reachability flag, and composite compliance risk scoring (OFAC + EU sanctions at bank-BIC level, + FATF). Pay per call in USDC via x402 ($0.003 to $0.02 per call) — no API key signup required. Also available as a native MCP server (npm `ibanforge-mcp`).
```
