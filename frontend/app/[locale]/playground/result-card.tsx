...
function BoolMark({ on }: { on: unknown }) {
  const t = useTranslations("playground.verdict")
  return <span className="inline-flex items-center gap-1.5">
    {on === true ? <Check aria-hidden className="size-3.5" style={{ color: "var(--ok)" }} /> : <Minus aria-hidden className="size-3.5" />}
    {on === true ? t("yes") : on === false ? t("no") : t("notProvided")}
  </span>
}
...
function SanctionMark({ hit }: { hit: unknown }) {
  ...
  return hit === true ? (
    ...
    <span className="inline-flex items-center gap-1.5">
      {hit === false ? t("noMatch") : t("notChecked")}
    </span>
  )
}
...
export function ResultCard({
  ...
  const statusLabel = isValidMode
    ? ok
      ? t("verdict.structureValid")
      : data.valid === false ? t("verdict.structureInvalid") : t("verdict.notChecked")
    : ok
      ? t("glance.found")
      : t("glance.notFound")
  ...
  return (
    <div key={animateKey} className="card-surface pg-result-in rounded-xl border overflow-hidden">
      ...
      {isValidMode ? <VerificationSummary data={data} mode={mode as "iban" | "compliance"} /> : null}

      {/* ── readable sections ─────────────────────────────────────────────── */}
      {ok ? (
        <details className="border-t border-[var(--hairline)]">
        <summary className="cursor-pointer p-4 text-sm font-medium sm:px-5">{t("verdict.details")}</summary>
...
function BannerPills({
  ...
  if ((mode === "iban" || mode === "compliance") && typeof sepa.member === "boolean") {
  ...
  // VoP
  // La participation VoP reste un détail du registre, jamais une pastille
  // susceptible d'être comprise comme une vérification du titulaire.
  ...
  return (
    <>
      ...
      <Section title={t("section.sepa")}>
        <Row k="Member"><BoolMark on={sepa.member} /></Row>
        <Row k="Schemes">{arr(sepa.schemes).map(String).join(" · ") || "—"}</Row>
        <Row k="VoP required"><BoolMark on={sepa.vop_required} /></Row>
      </Section>
      ...
      {str(cl.iid) && (
        <Section title={t("section.clearing")}>
          ...
          <Row k="SIC"><BoolMark on={cl.sic} /></Row>
          <Row k="Instant CHF"><BoolMark on={cl.instant_payments_chf} /></Row>
          <Row k="euroSIC"><BoolMark on={cl.eurosic} /></Row>
        </Section>
      )}
      <Section title={t("section.risk")}>
        <Row k="Issuer type">{str(ri.issuer_type)}</Row>
        <Row k="Country risk">{str(ri.country_risk)}</Row>
        <Row k="Test BIC"><BoolMark on={ri.test_bic} /></Row>
        <Row k="SEPA reachable"><BoolMark on={ri.sepa_reachable} /></Row>
      </Section>
    </>
  )
}
  ...
  return (
    <>
      <Section title={t("section.institution")}>
        ...
        <Row k="Test BIC"><BoolMark on={data.is_test_bic} /></Row>
      </Section>
  ...
  return (
    <>
      ...
      <Section title={t("section.sanctions")}>
        <Row k="Country"><SanctionMark hit={sanctions.country_sanctioned} /></Row>
        <Row k="Bank (BIC8)"><SanctionMark hit={sanctions.bank_sanctioned} /></Row>
  ...
  return (
    <>
      ...
      <Section title={t("section.services")}>
        <Row k="SIC"><BoolMark on={s.sic} /></Row>
        <Row k="RTGS CHF"><BoolMark on={s.rtgs_chf} /></Row>
        <Row k="Instant CHF"><BoolMark on={s.instant_payments_chf} /></Row>
        <Row k="euroSIC"><BoolMark on={s.eurosic} /></Row>
        <Row k="LSV BDD CHF"><BoolMark on={s.lsv_bdd_chf} /></Row>
        <Row k="LSV BDD EUR"><BoolMark on={s.lsv_bdd_eur} /></Row>
      </Section>
      <Section title={t("section.identifiers")}>
