...
Ohne Schlüssel, direkt aus dem Terminal: Die Kostprobe ohne Schlüssel beantwortet bis zu 25 Aufrufe pro Tag und Adresse, nur auf dieser Route.
...
Die Antwort ist vollständig, und sie enthält einen Block `trial`, der sagt, wie viele Aufrufe heute noch frei sind. Für den Betrieb nehmen Sie einen [Schlüssel ohne E-Mail](/de/docs/api-keys): 200 Anfragen im Monat, sobald er beansprucht ist, ohne Karte; wer eine ganze Datei prüfen will, nimmt [`POST /v1/iban/batch`](/de/docs/iban-validate) mit bis zu hundert IBANs pro Aufruf. Und wer keine Zeile Code schreiben möchte, findet [jede Bankleitzahl auf ihrer eigenen Seite](/de/blz), mit genau dieser Antwort darauf.
