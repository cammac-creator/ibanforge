...
[IBANforge](https://ibanforge.com) ist eine REST-API, die vollstaendige IBAN-Validierung -- Pruefsumme, Laenderregeln, BBAN-Parsing -- durchfuehrt und den BIC-Code in derselben Antwort aufloest, gegen einen ueber 121.000 Eintraege umfassenden konsolidierten BIC-Datensatz aus oeffentlichen Quellen (GLEIF, SWIFT-Verzeichnis, Bundesbank, SIX, NBP, EBA Step2 SCT), mit LEI-Anreicherung fuer ~38K Eintraege aus GLEIF. Eine POST-Anfrage, eine strukturierte JSON-Antwort.
...
- Das `bic`-Objekt enthaelt den Banknamen und die Stadt, aufgeloest aus dem konsolidierten BIC-Datensatz (GLEIF und weitere oeffentliche Quellen).
