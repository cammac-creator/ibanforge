...
| `POST /v1/iban/batch` | $0.002 USDC pro IBAN (1 IBAN = $0.002, 100 = $0.20) |
| `GET /v1/bic/:code` | $0.003 USDC |
| `POST /v1/iban/compliance` | $0.02 USDC |
| `GET /v1/ch/clearing/:iid` | $0.003 USDC |

Der Batch-Preis gilt pro IBAN und wird nach der Größe der gesendeten Liste
berechnet: ein Batch mit einer einzigen IBAN kostet $0.002, nicht den Preis
eines vollen Batches.
...
The same requirements travel in the `PAYMENT-REQUIRED` response header, base64
encoded. Clients that read the header and clients that read the body get the
same terms.

Amounts are atomic units: USDC has 6 decimals, so `"5000"` is $0.005. The
network is CAIP-2 — `eip155:8453` is Base mainnet.

> **Older v1 clients still settle.** We announce v2, but the paywall accepts a
> v1 `X-PAYMENT` signature as readily as a v2 `PAYMENT-SIGNATURE` one, so an
> integration written against the v1 shape keeps working.
