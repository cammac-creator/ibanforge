...
Dieses Beispiel zeigt die Antwort, wenn die EPC-Register und die Liste der Vereinten Nationen für den Aufruf nicht abgefragt wurden: `screened: false` bei `reachability` und `vop` sowie je ein `*_unavailable`-Flag pro ausgelassener Quelle, sodass ein `false` dort nichts beweist. Wurden sie abgefragt, steht `screened` auf `true` und die Werte sind die der Bank.
