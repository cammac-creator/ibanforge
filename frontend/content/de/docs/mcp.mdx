...
description: "IBANforge mit Claude, Cursor und jedem MCP-Agenten verbinden: das npm-Paket ibanforge-mcp, ein gehosteter Endpoint ohne Installation, elf Tools und Ergebnisse, die sagen, was als Nächstes zu tun ist."
order: 5
---
...
Nach dem Speichern den Client neu starten. Elf IBANforge-Tools erscheinen im Tool-Menü (neun auf dem gehosteten Endpoint unten — siehe Hinweis unter audit_status).
...
- Alle elf Tools liefern dasselbe JSON wie die REST-API, als Text und als MCP-`structuredContent` (neun davon auf dem gehosteten HTTP-Endpoint — siehe Hinweis unter audit_status).
