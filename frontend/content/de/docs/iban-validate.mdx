...
**Ganz ohne Schlüssel:** bis zu 25 Prüfungen pro Tag und Quelladresse (IPv6 pro /64 gezählt), nur auf dieser Route, zum Ausprobieren. Senden Sie eine echte `iban` ohne Zugangsdaten: Die Antwort ist vollständig und trägt einen Block `trial` mit der Zahl der heute verbleibenden Aufrufe. Danach antwortet die Route bis Mitternacht UTC mit `402` und `cause.reason: "trial_exhausted"`. Für alle anderen Endpunkte nehmen Sie einen [Schlüssel ohne E-Mail](/docs/api-keys).
