...
- **`basis: "national_register"`, `authoritative: true`** — das nationale Register des Landes veröffentlicht diesen BIC für diesen Bankcode. Heute sind das **Deutschland, Österreich, Belgien, Bulgarien und die Slowakei**: Die Bankleitzahlendatei der Bundesbank führt den exakten 11-stelligen BIC je BLZ, und die Register der OeNB, der Belgischen Nationalbank, das BAE-Register der Bulgarischen Nationalbank sowie der prevodník der Národná banka Slovenska veröffentlichen den BIC des Instituts je Bankcode. Das deutsche Register ist der Grund, weshalb eine Sparkasse zu ihrem eigenen BIC auflöst und nicht zum BIC8 der gemeinsamen Landesbank — das Feld benennt diese Zuordnung, es erzeugt sie nicht. Speicherbar und zur Abwicklung geeignet — mit einer Ausnahme: Ein Code, den das Register selbst als **zurückgezogen** führt, behält die Basis `national_register`, aber `authoritative` fällt auf `false`; lesen Sie `bank_code_check.retired` und `superseded_by`.
