...
| `basis` | `string` | Woher die Zuordnung Bankcode → BIC stammt. `national_register` — das nationale Register des Landes veröffentlicht diesen BIC für diesen Bankcode (heute DE, über die Bankleitzahlendatei der Bundesbank); `curated_map` — unsere gepflegte Bankcode-Karte, ein exakter Schlüssel und kein Vergabeakt; `directory_prefix` — der BIC-Präfix-Rückfall, der mehrere Institute treffen kann (siehe `bank_code_check.candidates`) |
...
- **`basis: "national_register"`, `authoritative: true`** — das nationale Register des Landes veröffentlicht diesen BIC für diesen Bankcode. Heute ist das **Deutschland**: Die Bankleitzahlendatei der Bundesbank führt den exakten 11-stelligen BIC je BLZ. Dasselbe Register ist der Grund, weshalb eine deutsche Sparkasse zu ihrem eigenen BIC auflöst und nicht zum BIC8 der gemeinsamen Landesbank — das Feld benennt diese Zuordnung, es erzeugt sie nicht. Speicherbar und zur Abwicklung geeignet.
