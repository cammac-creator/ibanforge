...
**Ein Fehler auf unserer Seite wird als `unavailable` gemeldet, nie als Verdikt.** Wenn die Referenzdaten nicht gelesen werden können — unlesbare Datenbank, nach einem fehlgeschlagenen Deploy fehlende Tabelle, Zeitüberschreitung einer Abfrage — lautet die Antwort `status: "unavailable"` mit `register: null`, genau die Form, die auch ein Land ohne jeden Datenbestand erhält. Nie `not_in_register`: Dieses Verdikt bedeutet „kein Institut hält diesen Code", und eine Störung bei uns ist kein Beleg über Ihren Empfänger. Beide Fälle werden gleich behandelt: weitermachen und einen Namensabgleich des Empfängers entscheiden lassen.
