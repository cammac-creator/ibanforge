...
## Vor dem Schlüssel: 10 Aufrufe pro Tag, ohne Schlüssel

`POST /v1/iban/validate` mit einer echten `iban` und ganz ohne Zugangsdaten wird vollständig beantwortet — samt Anreicherung — **10-mal pro Tag und IP-Adresse**, Rücksetzung um Mitternacht UTC:
...
Die Antwort enthält einen `trial`-Block mit der Zahl der heute verbleibenden Aufrufe und der Anfrage, die den Schlüssel erzeugt. Eine Kostprobe, keine Stufe: über 10 Aufrufe pro Tag antwortet der Endpunkt wieder mit `402` und `cause.reason: "trial_exhausted"` — der Schlüssel unten hebt die Grenze auf. Gezählt wird im Arbeitsspeicher je Serverinstanz: als Größenordnung zu verstehen, nie als Kontingent, auf dem man baut.
...
| Plan | Volumen | Kosten |
|---|---|---|
| Kostenlos (API-Schlüssel) | 200 Anfragen/Monat | $0 |
...
**Namensnennung im kostenlosen Tarif.** Jede Antwort eines kostenpflichtigen Endpunkts, die über einen kostenlosen Schlüssel läuft, trägt ein Objekt `attribution` (`text`, `url`, `note`). Wenn Sie diese Ergebnisse Menschen zeigen (eine Seite, ein Bildschirm, ein Dokument), blenden Sie „Powered by IBANforge" mit dem Link ein; ein Ergebnis, das in Ihrem Backend bleibt, schuldet nichts. Bezahlte Tarife tragen keine Namensnennung.
