...
| `404` | Not Found | Unbekannter Endpunkt. Ein unbekannter BIC ist kein 404: `/v1/bic/{code}` antwortet mit `200` und `found: false` |
...
| Code | Beschreibung |
|---|---|
| `invalid_json` | Anfrage-Body ist kein gültiges JSON |
| `invalid_request` | Das Feld `iban` fehlt oder ist keine Zeichenkette; bei einem Stapel enthält der Body kein Array von IBAN-Strings (`ibans`, `iban_list` oder `list`) |
| `empty_batch` | Das `ibans`-Array ist leer |
| `batch_too_large` | Mehr als 100 IBANs in einer Stapelanfrage |
| `invalid_bic_format` | BIC-Code hat nicht 8 oder 11 alphanumerische Zeichen |
...
Wird zurückgegeben, wenn der Zahlungs-Header fehlt, abgelaufen oder ungültig ist (`PAYMENT-SIGNATURE` für x402 v2, `X-PAYMENT` für v1). Die Antwort enthält Zahlungsanweisungen:
...
Der Body ist der reine Text `Internal Server Error`; ein 500 hat keine JSON-Hülle. Bitte melden Sie ihn mit Endpunkt und Uhrzeit des Aufrufs (UTC).
...
- Die BIC-Datenbank enthält über 121.000 BIC-Einträge aus öffentlichen Quellen (GLEIF, SWIFT-Verzeichnis, Bundesbank, SIX, NBP), mit LEI-Anreicherung für ~38K Einträge aus GLEIF, deckt aber möglicherweise nicht jede Filiale ab
