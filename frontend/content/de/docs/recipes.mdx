...
Das offizielle Java-SDK (Java 17 oder neuer, nur Jackson) spiegelt den TypeScript-Client Methode für Methode. Die Veröffentlichung auf Maven Central steht noch aus; bis dahin aus dem Repository bauen (`mvn install` in `sdks/java`) und `com.ibanforge:ibanforge-sdk:1.5.0` einbinden.
...
Das offizielle .NET-SDK (net8.0, keine Abhängigkeit ausserhalb des Frameworks) bietet dieselben Methoden als asynchrone Aufrufe. Die Veröffentlichung auf NuGet steht noch aus; bis dahin das Projekt referenzieren oder in `sdks/dotnet` `dotnet pack` ausführen (Paket-ID `IBANforge.Sdk`, Version 1.5.0).
