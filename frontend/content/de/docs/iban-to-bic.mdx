...
2. **Zuerst das Nationalregister fragen.** Für CH/LI (SIX BankMaster), DE (Bundesbank), AT (OeNB), BE (Belgische Nationalbank), BG (Bulgarische Nationalbank), SK (NBS) und CZ (ČNB) ist das Register die Vergabestelle: Ein Treffer ist das Institut, dem der Code gehört; ein Fehlen (`not_in_register`) heißt, der Code ist an niemanden vergeben — ein starker Grund, eine Zahlung zu stoppen. San Marino und Finnland werden ebenfalls gelesen, gehören aber nicht in diese Liste: Die Liste der operativen Banken der Zentralbank von San Marino nennt den Inhaber eines geführten Codes, und die Liste von Finance Finland ist eine Abschrift der Codes von Bankengruppen, die niemand aktualisiert (siehe [finnische Bankcodes](/docs/fi-bank-codes)); bei beiden löst ein Treffer den BIC auf, und ein Fehlen ist bloß ein Fehlen. Siehe [Datenquellen](/docs/data-sources) für das, was jedes Register veröffentlicht.
