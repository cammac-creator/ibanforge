...
This post is the recipe we would follow. It uses the [batch endpoint](/en/docs/iban-batch) because the unit of work is a list, not a payment, and it stops where an API has to stop: at the account itself.
...
If you want to see the fields on one supplier before writing any code, the [playground](/en/playground) runs the same call on a single IBAN.
