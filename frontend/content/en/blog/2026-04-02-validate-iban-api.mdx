...
[IBANforge](https://ibanforge.com) is a REST API that handles full IBAN validation — checksum, country rules, BBAN parsing — and resolves the BIC code in the same response, against a consolidated dataset of 121,000+ BIC entries from public sources (GLEIF, SWIFT directory, Bundesbank, SIX, NBP), with LEI enrichment for ~38K rows sourced from GLEIF. One POST request, one structured JSON response.
