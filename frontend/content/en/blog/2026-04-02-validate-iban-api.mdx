...
[IBANforge](https://ibanforge.com) is a REST API that handles full IBAN validation — checksum, country rules, BBAN parsing — and resolves the BIC code in the same response, against the 121,000+ entry GLEIF dataset used by central banks. One POST request, one structured JSON response.
