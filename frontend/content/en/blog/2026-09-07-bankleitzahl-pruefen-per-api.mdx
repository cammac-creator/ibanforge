...
description: "Checking a Bankleitzahl is not a checksum. The Deutsche Bundesbank publishes every month which codes are allocated, who holds them and which are being retired. Three real API answers, field by field; the keyless trial covers up to 25 checks a week."
date: "2026-09-07"
faq:
...
Without a key, straight from the terminal: the keyless trial serves up to 25 calls a week per address, on this route only.
...
The answer is complete, and it carries a `trial` block saying how many calls are left this week. For production, take a [key that needs no e-mail](/docs/api-keys): 200 requests a month once claimed, no card; to check a whole file, use [`POST /v1/iban/batch`](/docs/iban-validate) with up to a hundred IBANs per call. And if you would rather write no code at all, [every Bankleitzahl has its own page](/blz), with exactly this answer on it.
