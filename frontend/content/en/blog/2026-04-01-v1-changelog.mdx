...
- Covers **89 countries** with ISO 13616 IBAN formats
...
- **121,000+ BIC entries** from public sources (GLEIF, SWIFT directory, Bundesbank, SIX, NBP, EBA Step2 SCT)
- **LEI enrichment** — 38K rows include the Legal Entity Identifier (GLEIF-sourced)
- **Institution data** — name, city, country, and registration status
...
IBANforge ships with a built-in **Model Context Protocol (MCP) server**, exposing 4 tools for AI agents:
...
- `compliance_check` — Full compliance check: sanctions screening, SEPA Instant reachability, VoP participant check, risk score (0-100)
