...
The Bankleitzahlendatei carries the full eleven-character BIC per BLZ. That is why a Sparkasse resolves to its own BIC rather than to the shared BIC8 of its Landesbank: the pairing is published, we only relay it. In the response, `bic` carries the code and the bank name, and `sepa.basis: "national_register"` tells you the BIC was read from the register and not inferred. For a payout system that stores the BIC next to the IBAN, this is the value to store.
...
A free key covers 200 calls a month, enough to read every field on a sample before deciding anything. The full field reference is on [what "verified" means](/en/docs/iban-validate) and the German specifics on the [BLZ check](/en/docs/blz-check); Austria's register, which does carry an LEI, has [its own page](/en/docs/at-bank-codes).
