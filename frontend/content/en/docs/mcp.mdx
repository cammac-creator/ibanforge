...
Every validation result carries an ordered `next_steps` field: what blocks a payment comes first, what merely enriches it after. Each entry has a stable `code` to branch on, a `do` sentence the agent can relay, and a `because` naming the response field that produced it, so the advice is auditable rather than taken on trust. `bank_code_not_allocated` means stop, and so do `modulus_check_failed` (a UK sort code and account number that cannot be a real pair) and `national_check_digits_failed` (the national check key inside the IBAN does not match, so the account number cannot have been issued as written); `verify_payee_name` means carry on and let a beneficiary name check decide.
