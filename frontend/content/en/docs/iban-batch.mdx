...
Validate up to 100 IBANs in a single API call. Each IBAN is processed independently — one invalid IBAN does not affect the others. Paid per call in USDC via x402, an IBAN costs $0.002 in a batch instead of $0.005 on its own; on an API key or a credit pack, each IBAN uses one request or credit, the same as a single validation.
