...
| `POST /v1/iban/batch` | $0.002 USDC per IBAN (1 IBAN = $0.002, 100 = $0.20) |
| `GET /v1/bic/:code` | $0.003 USDC |
| `POST /v1/iban/compliance` | $0.02 USDC |
| `GET /v1/ch/clearing/:iid` | $0.003 USDC |

The batch price is per IBAN and billed on the size of the list you send, so a
one-IBAN batch costs $0.002, not the price of a full batch.
...
No minimum. You need enough USDC on Base to cover one request (as low as $0.003).
