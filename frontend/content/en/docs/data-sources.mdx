...
| **CZ** | Česká národní banka, Číselník kódů platebního styku v České republice (the directory of payment-system codes) | re-read daily; each **numbered edition** is published ahead of its effective date and applies **from that date, in Prague** — an edition announced but not yet in force waits beside the one in force, and `as_of` carries the effective date of the edition in force | name and BIC only, verbatim with Czech diacritics; no address. Building societies, credit unions and non-bank payment institutions hold codes too, several of them without a BIC |
...
A register counts only once its rows are in the database: until then, the country degrades to the composite map and `authoritative` stays `false` — the answer never claims a register it is not holding. The dated way to tell is `bank_code_check.register`, which names the register only once the rows are there.
...
Czech data is reproduced under the ČNB site terms ([*Podmínky užívání internetových stránek ČNB*](https://www.cnb.cz/cs/ochrana-osobnich-udaju-a-pravni-ujednani/podminky-uzivani-internetovych-stranek-cnb/), § 3, read 24/09/2026), which permit storing, passing on and reproducing the information published on its website, on the condition that the ČNB is always named as the source — in its own words, **Zdroj: ČNB** — and that an extract changes neither the facts nor their sense. Both travel with the answer: `bank_code_check.register` carries "Zdroj: ČNB" on every answer the register decides, refusals included, `bic.source` names the edition ("Zdroj: ČNB, Číselník kódů platebního styku v ČR, verze N"), `as_of` carries its effective date, and provider names are served exactly as published. **Reuse of the Czech fields carries the same conditions onward.** Why an absence is a refusal here: decree No. 169/2011 Coll. (*vyhláška č. 169/2011 Sb.*) makes IBAN positions 5-8 the payment code (§ 4 c) and has the ČNB publish every code it has allocated in this directory (§ 6 (2)). The directory publishes no successor code, so a Czech `not_in_register` never comes with a re-papering hint.
...
Finland left on 25 September 2026. Its hand transcription of the Finance Finland list, whose reuse terms the publisher never confirmed, was withdrawn together with the Finnish keys of our composite map: a Finnish bank code now answers `status: "unavailable"` with `reason: "no_reference_data_for_country"`. Not consulted, never refused. See [Finnish bank codes](/docs/fi-bank-codes).
...
FATF country-list data is used with the attribution its terms require, in the FATF's own citation format (the year and the access date move with each plenary sync):
...
The FATF permits commercial use of its data with credit; reuse of this API's FATF-derived fields carries the same attribution requirement onward.
