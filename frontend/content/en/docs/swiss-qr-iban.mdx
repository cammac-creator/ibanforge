...
This example shows the answer when the EPC registers and the United Nations list were not consulted for the call: `screened: false` on `reachability` and `vop`, and one `*_unavailable` flag per source left out, so a `false` there proves nothing. When they were consulted, `screened` is `true` and the values are the bank's own.
