...
2. **Ask the national register first.** For CH/LI (SIX BankMaster), DE (Bundesbank), AT (OeNB), BE (NBB), BG (Bulgarian National Bank), SK (NBS) and CZ (ČNB), the register is the allocation authority: a hit is the institution the code belongs to, a miss (`not_in_register`) means the code is allocated to nobody — a strong reason to stop a payment. San Marino and Finland are read too but do not belong in that list: the Central Bank of San Marino's list of operating banks names the holder of a code it carries, and the Finance Finland list is a transcription of the codes of banking groups that nothing refreshes (see [Finnish bank codes](/docs/fi-bank-codes)); for both, a hit resolves the BIC and a miss is just a miss. See [data sources](/docs/data-sources) for what each register publishes.
