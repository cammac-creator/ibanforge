...
2. **Ask the national register first.** For CH/LI (SIX BankMaster), DE (Bundesbank), AT (OeNB), BE (NBB), BG (Bulgarian National Bank), SK (NBS) and CZ (ČNB), the register is the allocation authority: a hit is the institution the code belongs to, a miss (`not_in_register`) means the code is allocated to nobody — a strong reason to stop a payment. San Marino is read too but does not belong in that list: the Central Bank of San Marino's list of operating banks names the holder of a code it carries, so a hit resolves the BIC and a miss is just a miss. Finland has no list any more (see [Finnish bank codes](/docs/fi-bank-codes)). See [data sources](/docs/data-sources) for what each register publishes.
