...
The official Java SDK (Java 17+, Jackson only) mirrors the TypeScript client method for method. Maven Central publication is pending; until then build it from the repository with `mvn install` in `sdks/java` and depend on `com.ibanforge:ibanforge-sdk:1.5.0`.
