...
| `404` | Not Found | Unknown endpoint. An unknown BIC is not a 404: `/v1/bic/{code}` answers `200` with `found: false` |
...
| `invalid_request` | The `iban` field is missing or not a string; for a batch, the body has no array of IBAN strings (`ibans`, `iban_list` or `list`) |
| `empty_batch` | The `ibans` array is empty |
| `batch_too_large` | More than 100 IBANs in a batch request |
| `invalid_bic_format` | BIC code is not 8 or 11 alphanumeric characters |
...
Returned when the payment header is missing, expired, or invalid (`PAYMENT-SIGNATURE` for x402 v2, `X-PAYMENT` for v1). The response includes payment instructions:
...
The body is the plain text `Internal Server Error` — there is no JSON envelope on a 500. Please report it with the endpoint and the time of the call (UTC).
...
- The BIC database contains 121k+ BIC entries from public sources (GLEIF, SWIFT directory, Bundesbank, SIX, NBP, EBA Step2 SCT) but may not cover every branch
