...
description: "Every HTTP status and error code of the IBANforge API: invalid IBANs, request errors, payment and quota (402), body size (413), rate limit (429), and where to get help."
order: 6
---

# Error Reference

IBANforge uses standard HTTP status codes and returns structured JSON errors, `{"error": "<token>", "message": "<sentence>"}`. Branch on `error`, which is stable; `message` may be reworded. This page covers every status and code you might encounter.
...
| `402` | Payment Required | A paid route called with no key, no payment and no free allowance left: the x402 payment envelope, with a `cause` when one applies |
...
These appear in the body of a `200` answer when `valid` is `false`: the request succeeded, the IBAN did not pass. `POST /v1/iban/validate`, each item of `POST /v1/iban/batch` and the free `GET /v1/iban/format` use the same six codes.
...
| `invalid_iban_length` | `GET` and `POST /v1/iban/format` | Fewer than 15 or more than 34 characters once spaces and hyphens are removed |
...
Returned when a paid route is called with no key, no payment and no free allowance left. The body is the x402 payment envelope; the `PAYMENT-SIGNATURE` header (x402 v2) or `X-PAYMENT` (v1) settles it. When a precise reason applies, the body also carries `cause.reason` and a `message` that names the way out:

| `cause.reason` | When |
|---|---|
| `trial_exhausted` | The keyless trial of `POST /v1/iban/validate` is used up for this address today; it comes back at midnight UTC |
...
Taking and claiming keys have their own daily limits, with their own `429` codes: see [API keys](/docs/api-keys). A key's monthly allowance is not a `429`: it ends in the `402` above, and every answer served on a key carries `X-Quota-Used`, `X-Quota-Limit` and `X-Quota-Remaining` (`GET /v1/keys/usage` returns the same count).
...
- Without a key, only `POST /v1/iban/validate` has a keyless trial (25 calls a day per source address). Every other paid route needs a key or a payment
