...
[IBANforge](https://ibanforge.com) est une API REST qui gere la validation complete d'IBAN -- somme de controle, regles pays, analyse BBAN -- et resout le code BIC dans la meme reponse, a partir d'un jeu de donnees BIC consolide de plus de 121 000 entrees issu de sources publiques (GLEIF, annuaire SWIFT, Bundesbank, SIX, NBP, EBA Step2 SCT), avec enrichissement LEI pour ~38K lignes provenant de GLEIF. Une requete POST, une reponse JSON structuree.
...
- L'objet `bic` inclut le nom de la banque et la ville, resolus a partir du jeu de donnees BIC consolide (GLEIF et autres sources publiques).
