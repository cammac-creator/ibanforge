...
description: "Un IBAN suisse passe trois contrôles, pas un : la clé de contrôle, l'IID à cinq chiffres contre le registre de SIX, et la question de savoir si c'est un QR-IBAN qui exige une référence QR. Trois vraies réponses de l'API, champ par champ ; l'essai sans clé couvre jusqu'à 25 vérifications par jour."
date: "2026-09-14"
faq:
...
Sans clé, depuis le terminal : l'essai sans clé sert jusqu'à 25 appels par jour et par adresse, sur cette seule route.
...
La réponse est complète et porte un bloc `trial` qui dit combien d'appels restent aujourd'hui. Pour la production, prenez une [clé sans e-mail](/fr/docs/api-keys) : 200 requêtes par mois une fois réclamée, sans carte. Qui ne veut écrire aucune ligne de code : [chaque IID a sa propre page](/fr/iid) avec exactement cette réponse, [le format de l'IBAN suisse avec l'exemple officiel](/fr/iban/ch) est à côté, et le [générateur d'IBAN de test](/fr/tools/test-iban) produit des IBAN suisses avec de vrais IID attribués, pour les fixtures et les démonstrations.
