...
Check compliance complet combinant validation IBAN, screening sanctions, accessibilite SEPA Instant, verification VoP et score de risque composite.
...
**Sortie :** Resultat de validation IBAN + statut du screening sanctions (OFAC/EU/ONU) + accessibilite SEPA Instant + verification participant VoP + score de risque (0-100). Cout : $0.02 USDC (identique a `/v1/iban/compliance`).
...
>
> Oui, cet IBAN est valide. Il appartient a **UBS Switzerland AG** a Zurich, Suisse — BIC UBSWCHZH, BC-Nummer suisse 00230, participant SIC avec paiements instantanes CHF.
...
- Les quatre outils renvoient les memes donnees structurees que l'API REST
