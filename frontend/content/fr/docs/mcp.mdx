...
description: "Connecter IBANforge à Claude, Cursor et tout agent MCP : le paquet npm ibanforge-mcp, un endpoint hébergé sans installation, un catalogue par transport, et des résultats qui disent quoi faire ensuite."
order: 5
---
...
La clé est optionnelle et gratuite — 25 requêtes/mois sans e-mail. [Générez-la](/docs/api-keys) avec un POST sans corps, puis envoyez `Authorization: Bearer ifk_...`. Réclamer cette même clé avec un code reçu par e-mail porte le quota à 200/mois, sans carte.

Après enregistrement, redémarrez le client. Le module npm expose **13 outils**, le serveur HTTP distant **11**. Les deux outils d’audit de fichiers sont réservés au module npm. Le [catalogue par transport](/agents#mcp-quickstart) détaille leur disponibilité.
...
Transport HTTP streamable, **25 appels d'outils gratuits par semaine et par adresse source**, sans aucune clé (semaine ISO en UTC, remise à zéro le lundi à 00:00 UTC ; un lot compte un appel par IBAN ; distinct de l'essai REST sans clé) : le chemin le plus rapide pour qu'un agent évalue la donnée avant tout engagement. Pour un usage suivi, lancez le paquet npm avec votre clé gratuite.

### L'ajouter comme connecteur dans un assistant de conversation

Dans un assistant de conversation, l'adresse ci-dessus devient un *connecteur personnalisé* : l'assistant appelle alors lui-même les outils MCP, au lieu de deviner une réponse qu'il ne peut pas obtenir.

**Un connecteur sans clé puise dans la même allocation sans clé que tout autre appelant**, comptée par adresse source. Dans Claude, les appels d'un connecteur partent du cloud d'Anthropic, pas de votre appareil (page officielle ci-dessous, section « Network requirements ») : cette allocation est donc partagée avec d'autres utilisateurs et peut être déjà épuisée quand vous commencez. Autres assistants : non vérifié par nous. L'endpoint hébergé ne lit aucune clé. Pour travailler avec votre propre clé, lancez le paquet npm dans un client de bureau avec `IBANFORGE_API_KEY` renseignée (Option 1 ci-dessus), ou appelez l'API REST avec la clé : un POST sur `https://api.ibanforge.com/v1/keys/generate` sans corps en renvoie une, sans e-mail.

Où cliquer, d'après la page officielle de chaque éditeur lue le 24 septembre 2026 ; les menus changent, et la page citée fait foi.

**Claude** ([page officielle](https://support.claude.com/en/articles/11175166), mise à jour le 11 août 2026). Les connecteurs personnalisés sont ouverts aux plans Free, Pro, Max, Team et Enterprise ; en Free, un seul connecteur personnalisé. *Non essayé par nous.*

- Plan individuel (la page donne ces étapes sous Pro et Max) : ouvrez **Customize > Connectors**, cliquez sur **+** puis **Add custom connector**, saisissez `https://api.ibanforge.com/mcp` et cliquez sur **Add**. Les champs OAuth de **Advanced settings** sont facultatifs ; ce serveur ne demande aucune authentification.
- Team ou Enterprise : un Owner ouvre **Organization settings > Connectors**, clique sur **Add**, survole **Custom** et choisit **Web**, saisit l'adresse et clique sur **Add** ; les membres le trouvent ensuite dans **Customize > Connectors** et cliquent sur **Connect**.
- Dans une conversation : le bouton **+** en bas à gauche, puis **Connectors**, pour l'activer.

**ChatGPT** ([page officielle](https://developers.openai.com/api/docs/guides/developer-mode)). Le mode développeur, sur le web, pour les comptes Pro, Plus, Business, Enterprise et Education : pas sur l'offre gratuite. *Non essayé par nous.*

- L'activer : **Settings → Security and login**, puis **Developer mode**.
- Ajouter le serveur : ouvrez [ChatGPT Plugins](https://chatgpt.com/plugins), choisissez le bouton plus et créez une application en mode développeur pour le serveur MCP distant `https://api.ibanforge.com/mcp`. La page cite le HTTP en flux parmi les protocoles et *No Authentication* parmi les options.
- Dans une conversation : choisissez **Developer mode** dans le menu plus, puis l'application.

**Gemini** ([page officielle](https://support.google.com/gemini/answer/17209137)). Les applications personnalisées demandent d'avoir 18 ans ou plus et d'être aux États-Unis, un compte Google personnel (pas un compte professionnel ou scolaire), l'activité conservée (Keep Activity) et l'anglais ; elles s'ajoutent dans l'application web de Gemini seulement, puis fonctionnent sur le web et dans l'application mobile. *Non essayé par nous.*

- Sur gemini.google.com, cliquez sur **Settings** en bas, puis **Connected Apps** ; sous « Custom apps », ajoutez une application personnalisée, saisissez `https://api.ibanforge.com/mcp`, cliquez sur **Next** et suivez les instructions à l'écran.
- La page demande des identifiants sous « Advanced features » quand un serveur ne prend pas en charge le Dynamic Client Registration. Ce serveur ne demande aucune authentification ; que Gemini l'accepte n'est pas vérifié par nous.
- Dans une requête : tapez **@** et choisissez l'application.
...
Chaque résultat de validation porte un champ `next_steps` ordonné : ce qui bloque un paiement d'abord, ce qui l'enrichit ensuite. Chaque entrée a un `code` stable sur lequel brancher, une phrase `do` que l'agent peut relayer, et un `because` qui nomme le champ de la réponse qui l'a produite — le conseil est auditable au lieu d'être cru sur parole. `bank_code_not_allocated` veut dire stop, tout comme `modulus_check_failed` (un sort code et un numéro de compte britanniques qui ne peuvent pas former une vraie paire) et `national_check_digits_failed` (la clé de contrôle nationale de l'IBAN ne correspond pas : le numéro de compte n'a pas pu être émis tel qu'il est écrit) ; `verify_payee_name` veut dire continuer et laisser trancher un contrôle du nom du bénéficiaire.
