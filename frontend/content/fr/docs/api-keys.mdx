...
## Avant la clé : 10 appels par jour, sans clé
...
La réponse porte un bloc `trial` avec le nombre d'appels restants aujourd'hui et la requête qui crée la clé. C'est une dégustation, pas un palier : au-delà de 10 appels par jour, l'endpoint répond de nouveau `402` avec `cause.reason: "trial_exhausted"`, et c'est la clé ci-dessous qui lève la limite. Le décompte vit en mémoire, par instance de serveur : à traiter comme un ordre de grandeur, jamais comme un quota sur lequel bâtir.
