...
`month` est le mois calendaire auquel se rapportent les compteurs (`YYYY-MM`). L'essai sans clé n'a pas de clé à consulter ici : ses compteurs sont hebdomadaires (semaine ISO en UTC, remise à zéro le lundi à 00:00 UTC), et dans son `402 trial_exhausted` le champ `cause.quota.month` vaut `week`, comme l'en-tête `X-Trial-Period`. Sans clé, cet endpoint répond `401 missing_key`, et avec une clé inconnue ou inactive `401 invalid_key` ; `GET /v1/keys/report`, `GET /v1/credits/balance` et `POST /v1/keys/claim` répondent de même. Cet endpoint est
