...
| `basis` | `string` | D'où vient l'appariement code banque → BIC. `national_register` — le registre national du pays publie ce BIC pour ce code banque (aujourd'hui DE, via la Bankleitzahlendatei de la Bundesbank) ; `curated_map` — notre carte de codes banque maintenue, une clé exacte et non un acte d'attribution ; `directory_prefix` — le repli par préfixe de BIC, qui peut correspondre à plusieurs établissements (voir `bank_code_check.candidates`) |
...
- **`basis: "national_register"`, `authoritative: true`** — le registre national du pays publie ce BIC pour ce code banque. Aujourd'hui, c'est l'**Allemagne** : la Bankleitzahlendatei de la Bundesbank porte le BIC exact à 11 caractères par BLZ. C'est ce même registre qui fait qu'une Sparkasse allemande se résout vers son propre BIC et non vers le BIC8 de la Landesbank partagée — le champ nomme cet appariement, il ne le crée pas. Stockable, et utilisable pour régler.
