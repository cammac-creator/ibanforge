...
| `basis` | `string` | D'où vient l'appariement code banque → BIC. `national_register` — le registre national du pays publie ce BIC pour ce code banque (aujourd'hui DE, AT, BE, SK, CZ, BG, CH, LI et SM) ; `curated_map` — notre carte de codes banque maintenue, une clé exacte et non un acte d'attribution ; `directory_prefix` — le repli par préfixe de BIC, qui peut correspondre à plusieurs établissements (voir `bank_code_check.candidates`) |
...
| `vop_participant` | `boolean \| null` | [Préparation VoP](/docs/vop) au niveau banque : `true` quand l'établissement résolu est listé comme prêt au registre du scheme VoP de l'EPC ; `null` quand aucun établissement n'est résolu, ou quand ce registre n'a pas été consulté pour l'appel |
...
- **`authoritative: true`** — le jeu de référence est le registre national lui-même. Aujourd'hui : **CH et LI** (SIX BankMaster), **DE** (Bankleitzahlendatei de la Bundesbank), **AT** (répertoire OeNB), **BE** (liste de codes de la Banque nationale de Belgique), **BG** (registre BAE de la Banque nationale bulgare — le verdict porte sur le code banque à quatre lettres, positions 5-8 de l'IBAN ; les chiffres d'agence ne sont pas vérifiés séparément), **SK** (le *prevodník* des codes d'identification du système de paiement domestique de la Národná banka Slovenska), **CZ** (le *číselník* des codes de paiement de la Česká národní banka : la loi tchèque en fait l'attribution des positions 5 à 8 de l'IBAN, et chaque édition s'applique à partir de sa propre date d'effet, pas du jour où nous la lisons). Dans ces pays, `not_in_register` signifie que le code n'est **pas attribué** — une raison forte d'arrêter un paiement. **Saint-Marin n'y figure délibérément pas** : la liste des banques opérationnelles de la Banque centrale nomme le titulaire d'un code qu'elle porte, donc un résultat est `verified` avec un bloc `institution`, mais elle ne publie pas l'attribution de l'espace ABI, donc une absence reste `absent_from_reference_data` et `authoritative` reste `false`. Voir [codes banque saint-marinais](/docs/sm-bank-codes). **La Finlande n'y figure pas non plus** depuis le 16 septembre 2026 : les codes Finance Finland sont une liste transcrite datée de 2025-10 que rien ne rafraîchit, attribués à des groupes bancaires plutôt qu'à des établissements ; un résultat est donc `verified` et nomme le groupe, tandis qu'une absence reste `absent_from_reference_data` avec `authoritative: false`. Voir [codes banque finlandais](/docs/fi-bank-codes).
...
### Clés de contrôle nationales (`checks.national_check_digits`)

Plusieurs pays gardent dans le BBAN une clé de contrôle qui leur est propre, plus ancienne que l'IBAN. C'est un second contrôle, indépendant du MOD-97 : un outil qui fabrique un IBAN autour d'un numéro de compte faux obtient sans effort des chiffres de contrôle IBAN justes, mais pas la clé nationale. IBANforge recalcule cette clé à partir de l'IBAN seul, dans le même appel et sans surcoût.

| Pays | `scheme` | La clé contrôlée |
|---|---|---|
| France, Monaco | `fr_rib_key` | La clé RIB, les deux derniers chiffres du BBAN, calculée sur le code banque, le code guichet et le numéro de compte (les lettres du numéro de compte sont converties selon la table du RIB) |
| Belgique | `be_mod97` | Les deux derniers chiffres : les dix premiers chiffres modulo 97, ou 97 quand le reste est nul |
| Italie, Saint-Marin | `it_cin` | Le CIN, la lettre de contrôle en tête du BBAN, calculée sur l'ABI, le CAB et le numéro de compte |
| Espagne | `es_dc` | Les deux chiffres DC, en positions 9 et 10 du BBAN |
| Royaume-Uni | voir `modulus_check` | Le contrôle modulus de Vocalink sur le sort code et le numéro de compte |

Pour ces six pays, un IBAN valide porte un bloc `national_check_digits`, et `checks.national_check_digits` en reprend le `status`. Dans les autres pays (hors Royaume-Uni), le contrôle vaut `not_checked` : les méthodes allemandes du numéro de compte, une par code banque, ne sont pas encore contrôlées.
...
- **`pass`** : la clé correspond, le numéro de compte est bien formé. Cela ne prouve pas que le compte existe ni qu'il est ouvert.
- **`fail`** : la clé ne correspond pas. Ce numéro de compte n'a pas pu être émis tel qu'il est écrit : une faute de frappe, ou un numéro inventé. `valid` reste `true`, parce que les chiffres de contrôle de l'IBAN sont justes ; lisez les deux séparément, et faites confirmer les coordonnées par le bénéficiaire avant de payer. `detail` dit quels chiffres ne concordent pas et ne donne jamais la clé attendue.
