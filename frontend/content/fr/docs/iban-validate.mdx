...
**Sans clé du tout :** jusqu'à 25 validations par jour et par adresse source (IPv6 comptée par /64), sur cette seule route, pour essayer. Envoyez un vrai `iban` sans identifiant : la réponse est complète et porte un bloc `trial` avec le nombre d'appels restants aujourd'hui. Au-delà, la route répond `402` avec `cause.reason: "trial_exhausted"` jusqu'à minuit UTC. Pour tous les autres endpoints, prenez une [clé sans e-mail](/docs/api-keys).
