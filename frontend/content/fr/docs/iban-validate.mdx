...
- **`basis: "national_register"`, `authoritative: true`** — le registre national du pays publie ce BIC pour ce code banque. Aujourd'hui, ce sont l'**Allemagne, l'Autriche, la Belgique, la Bulgarie et la Slovaquie** : la Bankleitzahlendatei de la Bundesbank porte le BIC exact à 11 caractères par BLZ, et les registres de l'OeNB, de la Banque nationale de Belgique, le registre BAE de la Banque nationale bulgare et le prevodník de la Národná banka Slovenska publient le BIC de l'établissement par code banque. C'est le registre allemand qui fait qu'une Sparkasse se résout vers son propre BIC et non vers le BIC8 de la Landesbank partagée — le champ nomme cet appariement, il ne le crée pas. Stockable, et utilisable pour régler — à une exception près : un code que le registre lui-même marque **retiré** garde la base `national_register` mais `authoritative` retombe à `false` ; lisez `bank_code_check.retired` et `superseded_by`.
