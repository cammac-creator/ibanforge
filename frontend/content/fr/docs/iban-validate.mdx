...
**Une défaillance de notre côté est signalée par `unavailable`, jamais par un verdict.** Lorsque les données de référence ne peuvent pas être lues — base illisible, table absente après un déploiement raté, requête qui expire — la réponse est `status: "unavailable"` avec `register: null`, exactement la forme servie pour un pays dont nous ne détenons rien. Ce n'est jamais `not_in_register` : ce verdict signifie « aucun établissement ne détient ce code », et une panne chez nous n'est pas une preuve sur votre bénéficiaire. Les deux cas se traitent pareil : continuez, et laissez un contrôle du nom du bénéficiaire trancher.
