...
Prendre et réclamer une clé ont leurs propres limites du jour, avec leurs propres codes `429` : voir [Clés API](/docs/api-keys). Le quota du mois d'une clé n'est pas un `429` : il se termine par le `402` ci-dessus. Chaque réponse servie sur une clé mensuelle (gratuite ou réclamée) porte `X-Quota-Used`, `X-Quota-Limit` et `X-Quota-Remaining` (`GET /v1/keys/usage` rend le même décompte) ; une clé à crédits prépayés porte à la place `X-Credits-Remaining` et `X-Credits-Total` (`GET /v1/credits/balance`).
