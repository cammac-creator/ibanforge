...
| `401` | Unauthorized | Les routes des clés seulement (`GET /v1/keys/usage`, `GET /v1/keys/report`, `GET /v1/credits/balance`, `POST /v1/keys/claim`) : `missing_key` ou `invalid_key`. Une route payante ne répond jamais `401` |
...
| `409` | Conflict | Les routes des clés seulement : `already_claimed`, `already_claimed_elsewhere`, `verification_in_flight`. Voir [Clés API](/docs/api-keys) |
...
| `502`, `503` | Bad Gateway, Service Unavailable | Le rail de paiement x402 est injoignable ou a répondu par une erreur. Réessayez plus tard. La route des établissements britanniques a ses propres codes `502` et `503`, sur [sa page](/docs/gb-firms) |
