...
| `404` | Not Found | Endpoint inconnu. Un BIC inconnu n'est pas un 404 : `/v1/bic/{code}` répond `200` avec `found: false` |
...
| `invalid_request` | Le champ `iban` manque ou n'est pas une chaîne ; pour un lot, le corps ne contient pas de tableau d'IBAN (`ibans`, `iban_list` ou `list`) |
| `empty_batch` | Le tableau `ibans` est vide |
| `batch_too_large` | Plus de 100 IBAN dans une requête batch |
| `invalid_bic_format` | Le code BIC ne fait pas 8 ou 11 caractères alphanumériques |
...
Renvoye lorsque l'en-tete de paiement est manquant, expire ou invalide (`PAYMENT-SIGNATURE` pour x402 v2, `X-PAYMENT` pour la v1). La reponse inclut les instructions de paiement :
...
Le corps est le texte brut `Internal Server Error` : un 500 n'a pas d'enveloppe JSON. Signalez-le avec l'endpoint et l'heure de l'appel (UTC).
...
- La base de donnees BIC contient plus de 121 000 entrees BIC issues de sources publiques (GLEIF, annuaire SWIFT, Bundesbank, SIX, NBP), avec enrichissement LEI pour ~38K lignes provenant de GLEIF, mais peut ne pas couvrir toutes les agences
