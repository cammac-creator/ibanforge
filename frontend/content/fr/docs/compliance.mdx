...
| `sanctions.fatf_status` | Position FATF du pays : `member`, `increased_monitoring` (liste grise), `high_risk` (liste noire) ou `none`. |
...
| `flags` | Les raisons du score, lisibles par machine, p. ex. `["country_sanctioned", "fatf_high_risk"]`. |
