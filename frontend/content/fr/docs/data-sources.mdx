...
| **CZ** | Česká národní banka, Číselník kódů platebního styku v České republice (répertoire des codes de paiement) | relu chaque jour ; chaque **édition numérotée** paraît avant sa date d'effet et ne s'applique **qu'à partir de cette date, heure de Prague** — une édition annoncée attend à côté de celle en vigueur, et `as_of` porte la date d'effet de l'édition en vigueur | le nom et le BIC seuls, verbatim avec les diacritiques tchèques ; aucune adresse. Des caisses d'épargne-logement, des coopératives d'épargne et de crédit et des établissements de paiement non bancaires détiennent aussi des codes, plusieurs sans BIC |
...
Un registre ne compte qu'une fois ses lignes dans la base : jusque-là, le pays retombe sur la carte composite et `authoritative` reste `false` — la réponse ne revendique jamais un registre qu'elle ne détient pas. Le repère fiable est `bank_code_check.register`, qui ne nomme le registre qu'une fois les lignes présentes.
...
Les données tchèques sont reproduites au titre des conditions d'usage du site de la ČNB ([*Podmínky užívání internetových stránek ČNB*](https://www.cnb.cz/cs/ochrana-osobnich-udaju-a-pravni-ujednani/podminky-uzivani-internetovych-stranek-cnb/), § 3, lues le 24/09/2026), qui autorisent à stocker, transmettre et reproduire les informations publiées sur son site, à condition que la ČNB soit toujours citée comme source — selon ses propres mots, **Zdroj: ČNB** — et qu'un extrait ne change ni les faits ni leur sens. Les deux voyagent avec la réponse : `bank_code_check.register` porte « Zdroj: ČNB » sur chaque réponse que le registre décide, refus compris, `bic.source` nomme l'édition (« Zdroj: ČNB, Číselník kódů platebního styku v ČR, verze N »), `as_of` porte sa date d'effet, et les noms de prestataires sont servis exactement comme publiés. **La réutilisation des champs tchèques emporte les mêmes conditions.** Pourquoi une absence vaut refus ici : le décret n° 169/2011 Sb. (*vyhláška č. 169/2011 Sb.*) fait des positions 5 à 8 de l'IBAN le code de paiement (§ 4 c) et charge la ČNB de publier dans ce répertoire chaque code qu'elle a attribué (§ 6 al. 2). Le répertoire ne publie aucun code successeur : un `not_in_register` tchèque n'est jamais accompagné d'une piste de re-papérisation.
...
- un code listé répond `verified` avec l'établissement que la Banque centrale nomme — et `authoritative: false` ;
- un code absent de la liste répond exactement ce que Saint-Marin répondait avant ce registre : `not_in_register` avec la raison **`absent_from_reference_data`**. Jamais `not_allocated`, et jamais une raison d'arrêter un paiement.
...
Source : **Bank of England (List of Banks, 2026-09)**, la liste mensuelle des établissements que la Prudential Regulation Authority agrée pour recevoir des dépôts. Utilisée avec la permission écrite de la Bank of England, dont la condition est l'attribution à la Bank **et au mois de publication de la liste** — c'est pourquoi chaque bloc `pra_authorisation` porte `list_month`, lu de la donnée chargée et jamais écrit à la main.
...
Les listes de pays du GAFI sont utilisées avec l'attribution que ses conditions exigent, dans le format de citation du GAFI lui-même (l'année et la date de consultation changent à chaque mise à jour après une plénière) :
...
Le GAFI permet l'usage commercial de ses données avec crédit ; toute réutilisation des champs GAFI de cette API doit porter la même exigence d'attribution.
