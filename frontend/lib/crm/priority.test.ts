...
describe('priorityOf', () => {
  ...
  it('ranks a paying client above a warm prospect', () => {
    const paying = priorityOf(
      client('a@example.net', { apiKey: { keyPrefix: 'k', paid: true, creditsTotal: 5000, creditsRemaining: 10, monthlyLimit: null, usedAllTime: 40, lastActiveMonth: '2026-07', createdAt: null, issuedByUs: false, isNew: false } }),
      situation(),
    );
  ...
  it('counts a free key that is genuinely used as a client', () => {
    const used = priorityOf(
      client('a@example.net', { apiKey: { keyPrefix: 'k', paid: false, creditsTotal: null, creditsRemaining: null, monthlyLimit: 1000, usedAllTime: 231, lastActiveMonth: '2026-07', createdAt: null, issuedByUs: false, isNew: false } }),
      situation(),
    );
  ...
  it('still ranks a converted prospect as a client once the key is used', () => {
    const active = client('a@example.net', {
      ...
      apiKey: { keyPrefix: 'k', paid: false, creditsTotal: null, creditsRemaining: null, monthlyLimit: 1000, usedAllTime: 900, lastActiveMonth: '2026-07', createdAt: null, issuedByUs: false, isNew: false },
    });
  ...
  it('gives every rung a reason to show on the row', () => {
    ...
    reasons.add(priorityOf(client('a@example.net', { apiKey: { keyPrefix: 'k', paid: true, creditsTotal: 1000, creditsRemaining: 1, monthlyLimit: null, usedAllTime: 1, lastActiveMonth: null, createdAt: null, issuedByUs: false, isNew: false } }), situation()).reason);
