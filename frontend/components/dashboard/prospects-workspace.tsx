  ...
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-[320px_1fr]">
      ...
      <div className="rounded-xl border border-[var(--ink-4)]/60 bg-[var(--ink-2)]/40">
        <div className="space-y-2 border-b border-[var(--ink-4)]/60 p-3">
          ...
          <div className="flex flex-wrap gap-1">
            {(
              ...
              <button
                ...
                className={`rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${statusFilter === k ? 'bg-amber-500/15 text-amber-400' : 'text-[var(--fg-4)] hover:text-[var(--fg-2)]'}`}
              >
                {lbl}
              </button>
            ))}
          ...
          <select
            ...
            className="w-full rounded-lg border border-[var(--ink-4)] bg-[var(--ink-0)] px-2 py-1.5 text-xs text-[var(--fg-2)] focus:border-amber-500/40 focus:outline-none"
          >
        ...
        <div className="max-h-[70vh] overflow-y-auto">
          {filtered.length === 0 && <p className="p-4 text-sm text-[var(--fg-5)]">Aucun prospect.</p>}
          {filtered.map((p) => {
            const ds = displayStatus(p);
            const st = PSTATUS[ds];
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className={`truncate text-sm ${unread ? 'font-bold text-white' : 'font-medium text-[var(--fg-1)]'}`}>{p.company}</span>
                  {p.country && <span className="ml-auto shrink-0 text-[10px] text-[var(--fg-5)]">{p.country}</span>}
                </div>
                <div className="flex items-center gap-2 pl-4">
                  ...
                  <span className="truncate text-[10px] text-[var(--fg-5)]">{sg.label}</span>
                </div>
              </button>
            );
          })}
      ...
      <div className="rounded-xl border border-[var(--ink-4)]/60 bg-gradient-to-br from-[var(--ink-2)] to-[var(--ink-2)]/60 p-5">
        {!sel ? (
          <div className="flex h-64 items-center justify-center text-sm text-[var(--fg-5)]">Sélectionne un prospect à gauche.</div>
        ) : (
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="text-[10px] uppercase tracking-wide text-[var(--fg-4)]">{label}</p>
      <p className="mt-0.5 text-sm text-[var(--fg-2)]">{children || '—'}</p>
    </div>
  );
}
...
function ProspectDetail({ p }: { p: Prospect }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-lg border border-[var(--ok)]/20 bg-[var(--ok)]/5 px-3 py-2">
        <p className="text-[10px] uppercase tracking-wide text-[var(--ok)]/70">Signal d’achat</p>
        <p className="mt-0.5 text-sm text-[var(--fg-1)]">{p.buying_signal || '—'}</p>
        {p.signal_source_url && (
          <a href={p.signal_source_url} target="_blank" rel="noreferrer" className="mt-1 inline-block text-[11px] text-[var(--ok)] hover:underline">
            preuve ↗
          </a>
        )}
      </div>
      ...
      {/* contact */}
      <div className="rounded-lg border border-[var(--ink-4)] bg-[var(--ink-0)]/50 px-3 py-2 text-sm">
        <p className="text-[10px] uppercase tracking-wide text-[var(--fg-4)]">Contact</p>
        <p className="mt-0.5 text-[var(--fg-1)]">
          {p.contact_name || '—'}
        ...
        {p.personalization_hook && (
          <p className="mt-1 text-[11px] text-[var(--fg-4)]">
            <span className="text-[var(--fg-3)]">Accroche :</span> {p.personalization_hook}
          </p>
        )}
      </div>
...
function ProspectTimeline({ p }: { p: Prospect }) {
  if (!p.messages.length) {
    return (
      <div className="border-t border-[var(--ink-4)]/60 pt-4">
        <p className="mb-1 text-[11px] uppercase tracking-wide text-[var(--fg-4)]">Conversation</p>
        <p className="text-sm text-[var(--fg-5)]">
  ...
  return (
    <div className="border-t border-[var(--ink-4)]/60 pt-4">
      <p className="mb-3 text-[11px] uppercase tracking-wide text-[var(--fg-4)]">Conversation ({p.messages.length})</p>
...
function ProspectComposer({ p }: { p: Prospect }) {
  ...
  if (noMail) {
    return (
      <div className="border-t border-[var(--ink-4)]/60 pt-4 text-sm text-amber-300/90">
  ...
  return (
    <div className="border-t border-[var(--ink-4)]/60 pt-4">
      <div className="mb-2 flex items-center justify-between">
        <p className="text-[11px] uppercase tracking-wide text-[var(--fg-4)]">Mail prêt — relis, ajuste, envoie</p>
        <div className="flex gap-1">
          {(['fr', 'en'] as const).map((l) => (
            <button
              ...
              className={`rounded px-2 py-0.5 text-[11px] font-medium transition-colors ${lang === l ? 'bg-amber-500/15 text-amber-400' : 'text-[var(--fg-4)] hover:text-[var(--fg-2)]'}`}
            >
              {l.toUpperCase()}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3">
        ...
        <span className="text-[11px] text-[var(--fg-4)]">{p.contact_email ? `→ ${p.contact_email}` : 'email manquant'}</span>
      </div>
...
function StatusActions({ p }: { p: Prospect }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 border-t border-[var(--ink-4)]/60 pt-3 text-[11px]">
      <span className="text-[var(--fg-5)]">Classer :</span>
      <button type="button" disabled={busy} onClick={() => setStatus('archive')} className="text-[var(--fg-4)] hover:text-[var(--fg-2)] disabled:opacity-50">
        archiver
      </button>
      <button type="button" disabled={busy} onClick={() => setStatus('rejete')} className="text-[var(--fg-4)] hover:text-red-400 disabled:opacity-50">
        rejeter
      </button>
      {p.status !== 'a_mailer' && p.contact_email && (
        <button type="button" disabled={busy} onClick={() => setStatus('a_mailer')} className="text-[var(--fg-4)] hover:text-[var(--ok)] disabled:opacity-50">
          marquer à mailer
        </button>
      )}
    </div>
