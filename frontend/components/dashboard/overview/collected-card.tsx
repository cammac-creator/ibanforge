...
/**
 * La tuile « Encaissé » : ce que Stripe a encaissé pour IBANforge, par nature
 * (packs, abonnements, audits), et l'heure de la lecture. Les autres paiements
 * du compte Stripe sont montrés à part, jamais dans le titre ; le net, le viré
 * et l'en-attente couvrent le compte entier, et la ligne le dit dès qu'il y a
 * d'autres paiements. Si Stripe ne répond pas, le total selon nos propres
    ...
    <StatCardV2
      ...
      detail={
        <>
          ...
          {v.other !== null && (
            <p data-collected-other className="text-amber-300">{t('money.collected.other', { amount: v.other })}</p>
          )}
          {fromStripe ? (
            <p data-collected-settlement>
              {v.accountWide && <>{t('money.collected.accountWide')}{' '}</>}
