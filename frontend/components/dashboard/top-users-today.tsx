    ...
    <div className="rounded-xl border border-amber-500/20 bg-gradient-to-br from-[var(--ink-2)] to-[var(--ink-2)]/60 px-4 py-3">
      <div className="flex flex-wrap items-baseline justify-between gap-x-2 gap-y-1">
        <div className="flex items-center gap-1.5">
          ...
          <h2 className="text-[11px] font-semibold uppercase tracking-wider text-[var(--fg-2)]">Top utilisateurs du jour</h2>
          <InfoDot>
            ...
            return (
              <div key={u.email} className={`rounded-lg border px-2.5 py-1.5 ${st.border} ${st.bg}`}>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="truncate text-[13px] font-medium text-[var(--fg-1)]" title={u.email}>
                    {u.company ?? u.email}
                  </span>
                ...
                <div className="mt-0.5 flex items-center gap-1.5 pl-6 text-[9px] uppercase tracking-wide">
                  ...
                  <span className="text-[var(--fg-5)]">{u.period === 'today' ? 'req aujourd’hui' : '📅 req ce mois-ci'}</span>
                </div>
              </div>
            );
          })}
