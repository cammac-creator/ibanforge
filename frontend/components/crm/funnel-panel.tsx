...
function Table({ title, hint, rows }: { title: string; hint?: string; rows: FunnelRow[] }) {
  ...
  return (
    <div className="min-w-0">
      <h3 className="text-[11px] font-semibold uppercase tracking-wide text-[var(--fg-2)]">{title}</h3>
      {hint && <p className="mt-0.5 text-[10px] text-[var(--fg-3)]">{hint}</p>}
      ...
      <div className="mt-1.5 overflow-x-auto">
        <table className="w-full min-w-[30rem] border-collapse text-[11px]">
          <thead>
            <tr className="text-left text-[10px] uppercase text-[var(--fg-3)]">
  ...
  return (
    <details className="min-w-0 rounded-xl border border-[var(--ink-4)]/60 bg-[var(--ink-2)]/40">
      <summary className="cursor-pointer list-none px-4 py-2.5 text-sm font-medium text-[var(--fg-1)] marker:content-none">
        ...
        <span className="ml-2 text-[11px] font-normal text-[var(--fg-3)]">
